...
#}
#
### Vault K/V 저장된 비밀 번호 조회하는 데이터 소스
#data "vault_generic_secret" "pwd_gen" {
#    for_each = local.db_ids
#    path = "kv/channel/${each.key}"
...
#output "pwd_gen" {
#   value = [for pwd in local.db_ids: nonsensitive(data.vault_generic_secret.pwd_gen[pwd].data["password"])]   
#}
#
#
#
...
resource "null_resource" "enc_pwd" {
    ...
    #for_each =[for pwd in local.db_ids: nonsensitive(data.vault_generic_secret.pwd_gen[pwd].data["password"])]   
